...
%
The first objective of this project was to select a functional and basis set combination which worked well across \textit{cis}-AB, \textit{trans}-AB and tolane which are the components of the molecules we are interested in. To accomplish this task we calculated the 20 energetically lowest excited states of each molecule with four different functionals (B97, B97M-rV, CAM-B3LYP 
and $\omega$B97M-V) and eight different basis sets (3-21G, 3-21G*, 6-31G, 6-31G*, 6-311G, 6-311G*, def2-SVPD and def2-TZPD). After calculating the natural transition orbitals (ntos) of those three molecules we assigned the energetically lowest electronic states to their specific transition. The first and second excited state resulting from each calculation were compared with the experimental values for the first and second peak of the respective UV-spectrum.\cite{AB_5, tol_uv_spec}  \\
%
%
%
...
For \textit{cis}-AB, the CAM-B3LYP functional in combination with the 6-311G$^{*}$ basis set yielded a satisfying result in relation to the minimal relative error and their respective computational cost. The calculated transition energies which where obtained via the CAM-B3LYP functional as well as the experimentally derived values are shown in table \ref{tab:cAB-Exp-calc}.
...
%
\begin{table}[H]
    \caption{Type of transition and vertical excitation energies of \textit{cis}-AB, calculated with CAM-B3LYP\//6-311G* with D3 correction $(E_{\text{calc}})$ and experimentally obtained ($E_{\text{exp}}$).}
...
\end{table}%
%
It appears that the calculated excitation energies tend to be marginally higher than the experimentally obtained ones. The full UV-spectrum of \textit{cis}-azobenzene 
%
%
...
\end{subfigure}
%
\caption{Detachment (red) and attachment (blue) densities for the energetically lowest five excited singlet states S1 to S5 of tolane at the CAM-B3LYP/\\6-311G* level of theory with an isovalue of 0.02.}
