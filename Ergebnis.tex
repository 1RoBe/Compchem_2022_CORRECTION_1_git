...
%
The resulting states were fitted with a Lorentzian function resulting in figure \ref{fig:uv_fig_cAB}, figure \ref{fig:uv_fig_tAB} and figure \ref{fig:uv_fig_tolane}, which were calculated at the CAM-B3LYP/6-311G* level of theory with the D3(BJ) dispersion correction.
To assign transitions to the respective singlet execited states, natural transition orbitals (ntos) were calculated at the CAM-B3LYP/6-311G* level of theory with the D3(BJ) dispersion correction for cAB, tAB and tolane.\\
The energetically lowest singlet excited state (S1) of \textit{cis}-azobenzene shown in figure \ref{fig:uv_fig_cAB} can be assigned to the n$\pi^*$ transition, while S2-S5 are $\pi\pi^*$ transitions. 
%
%
\begin{table}[H]
...
\midrule
S1    & n$\pi^*$		& 431			& 2.877				              & 0.03\\
S2    & $\pi\pi^*$ 		& \textbf{256}	& \textbf{4.837}	  & \textbf{0.14} \\
S3    & $\pi\pi^*$ 		& 249			& 4.973                   & 0.02          \\
S4    & $\pi\pi^*$		& 243			& 5.109                   & 0.08          \\
S5    & $\pi\pi^*$		& 241			& 5.152                   & \SI{1.58e-5}{}       \\
\bottomrule
\end{tabular}
\end{table}
% \SI{}{}
...
\end{subfigure}
%
\caption{Detachment (red) and attachment (blue) densities for the energetically lowest five excited singlet states S1 to S5 of cAB at the CAM-B3LYP/6-311G* + D3(BJ) with level of theory with an isovalue of 0.02.}
\label{fig:nto_cab}
\end{figure}
\vspace{1 cm}
%
...
%
%
Similar to cAB, the first singlet excited state (S1) can be assigned to the n$\pi^*$ transition. The small intensity can be explained by the symmetry-forbidden nature of the n$\pi^*$ transition. States S2-S5 can be assigned to $\pi\pi^*$ transitions (figure \ref{fig:nto_tab}).
%
%
%
\begin{table}[H]
...
State & Trans. & $\lambda$ (nm)     & $E$ $\left(\si{\eV}\right)$          & $f$    \\
\midrule
S1    & n$\pi^*$	& 439             & 2.824                  & \SI{5.41e-5}{}        \\
S2    & $\pi\pi^*$ 	&\textbf{284}	  & \textbf{4.372}         & \textbf{1.04} \\
S3    & $\pi\pi^*$	& 256             & 4.847                  & 0.03         \\
S4    & $\pi\pi^*$	& 255             & 4.857                  & \SI{1.80e-5}{}          \\
S5    & $\pi\pi^*$	& 219             & 5.664                  & \SI{2.19e-5}{}         \\
\bottomrule
\end{tabular}
\end{table}
%
...
\end{subfigure}
%
\caption{Detachment (red) and attachment (blue) densities for the energetically lowest five excited singlet states S1 to S5 of tAB at the {CAM-B3LYP/6-311G* + D3(BJ)} level of theory with an isovalue of 0.02.}
\label{fig:nto_tab}
\end{figure}
\vspace{1 cm}
%
...
%
%
Figure \ref{fig:uv_fig_tolane} shows the UV-spectrum of tolane, while the energies of the singlet excited states of tolane are shown in table \ref{tab:excited_states_tolane} and their ntos are shown in figure \ref{fig:nto_tolane}.\\
All five singlet excited states can be assigned to $\pi\pi^*$ transitions.
%
%
%
\begin{table}[H]
...
State & Trans. & $\lambda$ (nm)     & $E$ $\left(\si{\eV}\right)$          & $f$    \\
\midrule
S1    & $\pi\pi^*$	& \textbf{263}    & \textbf{4.7061}        & \textbf{1.12} \\
S2    & $\pi\pi^*$	& 238             & 5.2116                 & \SI{3.70e-5}{}         \\
S3    & $\pi\pi^*$	& 238             & 5.2184                 & \SI{1.59e-6}{}          \\
S4    & $\pi\pi^*$	& 233             & 5.3175                 & \SI{1.00e-7}{}         \\
S5    & $\pi\pi^*$	& 199             & 6.2348                 & \SI{4.66}{}         \\
\bottomrule
\end{tabular}
\end{table}
%
...
The CAM-B3LYP functional in combination with the 6-311G$^{*}$ basis set yielded a satisfying result for cAB in relation to the minimal relative error and their respective computational cost.
...
In the case of \textit{trans}-AB, CAM-B3LYP performed quite well in combination with the 6-311G* basis set and is only slightly outperformed by B97 and B97M-rV as seen in Figure \ref{fig:tAB_Benchmark}.
...
The benchmark for tolane yielded an interesting result. It appears that the higher rung functionals, CAM-B3LYP and $\omega$B97M-V, perform worse than their lower rung counterparts especially in bigger basis sets (def2-SVPD, def2-TZVP). 
...
%
\begin{table}[H]
\caption{Type of transition and vertical excitation energies of tolane, calculated at the CAM-B3LYP/6-311G* + D3(BJ) level of theory $(E_{\text{calc}})$ and experimentally obtained ($E_{\text{exp}}$).}
...
At first this was done by manually creating raw geometries that were then further refined using DFT. 
...
To accomplish this, we decreased the CNNC dihedral angle of tAB-1c-tolane-rot9 and tAB-3c-tolane-rot3 such that the lowest amount of torsion is required to reach \SI{0}{\degree}.
% This was accomplished by decreasing the absolute value of the CNNC dihedral angle of torsion of tAB-1c-tolane-rot9 and tAB-3c-tolane-rot3, such that the required amount of torsion to reach 0\si{\degree} is minimized,
% e.g. the minimal amount of steps are needed to hypothetically reach 0\si[]{\degree}.
The CNNC dihedral angle was varied in steps of 20\si{\degree} and at each step a geometry optimisation was performed at the CAM-B3LYP/6-311G* + D3(BJ) level of theory.
Since the calculations didn't succeed after a dihedral angle of 100\si[]{\degree} was reached, 
we had to manually decrease the dihedral angle to about 80\si[]{\degree}. 
At this point we performed an unconstrained geometry optimisation at the CAM-B3LYP/6-311G* + D3(BJ) level of theory that yielded the \textit{cis}-isomers seen in figure~\ref{fig:Conformers}.
%
%
...
\label{abs:exc_state_analysis}
To further characterize the molecules we calculated their lowest lying singlet excited states and visualized their natural transition orbitals (ntos) as well as their molecular orbitals (MOs), which are shown in the appendix.
...
\\
Similar to tAB the second singlet excited state (S2) is a $\pi \pi^{*}$-transition, which is slightly red shifted in comparison to tAB ($\SI{4.341}{\eV}$, $\SI{4.372}{\eV}$ for tAB) and possesses less than half the oscillator strength compared to S2 of tAB (0.38, 1.04 for tAB). \\
...
\\
To summarize, it appears that the excited states of tAB-1c-tolan-crest-rot9 are very similar to those found in their fragments (tAB and tolane) and the linkage didn't lead to new states which contribute in a major way to the resulting UV-spectrum.
%
\begin{figure}[H]
  \centering
  ...
  %
  %
  \caption{Detachment (red) and attachment (blue) densities at an isovalue of 0.02 for the energetically three lowest excited singlet states S1 to S3 of tAB-1c-tolane-crest-rot9 at the CAM-B3LYP/6-311G* + D3(BJ) level of theory}
  ...
  \end{subfigure}
  %
  \caption{Detachment (red) and attachment (blue) densities at an isovalue of 0.02 for the  excited singlet states S4 to S6 of tAB-1c-tolane-crest-rot9 at the CAM-B3LYP/6-311G* + D3(BJ) level of theory}
  \label{fig:att-det:tAB-1c-tolane-crest-rot9}
  \end{figure}
%
%
...
\begin{figure}[h]
  ...
  \end{subfigure}
  %
  \caption{Detachment (red) and attachment (blue) densities with an isovalue of 0.02 for the energetically lowest six excited singlet states S1 to S6 of tAB-3c-tolane-crest-rot3 at the CAM-B3LYP/6-311G* + D3(BJ) level of theory.}
...
The attachment and detachment densities are located on the azo group.\\
%
%
%
...
\\
The $\pi\pi^{*}$ S2 state can be compared to the first excited singlet state of tolane, since the attachment and detachment densities are located on the tolane subunit.
...
% This exited singlet state is quite similar to the S3 of cAB, but possesses a greater intensity (0.32, 0.02 for cAB) and is shifted to a longer wavelength (\SI[]{4.71}{\electronvolt}, \SI[]{4.97}{\electronvolt} for cAB).
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{Figures/Conformer_UV_spectra_ink/cAB-1c-tolan-abs_mod-tor.eps}
    \caption{UV spectra of cAB-1c-tolan-tor (black) and a combination of cAB and tolane (red). The bars signify the strength and wavelength of the individual transitions.}
...
\begin{figure}[H]
  ...
  %
  %
  \caption{Detachment (red) and attachment (blue) densities at an isovalue of 0.02 for the energetically lowest three excited singlet states S1 to S3 of cAB-1c-tolane at the CAM-B3LYP/6-311G* + D3(BJ) level of theory.}
  ...
  \end{subfigure}
  %
  \caption{Detachment (red) and attachment (blue) densities at an isovalue of 0.02 for the excited singlet states S4 to S6 of cAB-1c-tolane at the CAM-B3LYP/6-311G* + D3(BJ) level of theory.}
  \label{fig:att-det:cAB-1c-tolane-crest-rot9}
  \end{figure}
...
S1 can be assigned to the n$\pi^*$ transition. \\
\\
% 
% \begin{figure}[H]
%   \centering
...
The attachment density of S3 is mostly located at the n-orbitals of the azo group while the attachment densities of S4 and S5 include one of the adjacent phenyl groups.\\
\\
...
\end{subfigure}
%
\caption{Detachment (red) and attachment (blue) densities at an isovalue of 0.02 for the energetically lowest four excited singlet states S1 to S6 of cAB-3c-tolane-tor at the CAM-B3LYP/6-311G* + D3(BJ) level of theory.}
...
\label{abs:iso_process}
To gain further insight into the trans-cis isomerisation of the linked molecules, we investigated the reactive coordinate of the isomerisation (the CNNC dihedral).
% the effect of torsion on the electronic states. 
The CNNC dihedral angle was varied in steps of \SI{20}{\degree} (figure \ref{fig:tors_tAB-cAB-1c-pathway} and \ref{fig:tors_tAB-cAB-3c-pathway}) and a respective constrained optimisation of the ground state was performed. 
The energetically lowest 20 singlet excited states were then calculated and plotted against the torsional angle. 
%
%
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{Figures/Torsion_s0_s1_s2/Pictures_AB-1c-tors_corr_2.eps}
    \caption{Relative energies of the GS and the singlet excited states in in relation to the CNNC dihedral angle of tAB-1c-tolane-rot9 and cAB-1c-tolane-tor.
    S1-S6 describe the singlet excited states of tAB-1c-tolane-rot9.}
...
and the GS energy of tAB-1c-tolane-rot9 with a CNNC dihedral angle of \SI[]{169}{\degree} is used as a reference point.\\
% The singlet excited states S1 (orange) and the S2 (red) can be assinged ot transitions which take place at the AB substructure of the linked molecules. 
% S6 (green) is a transition which is located at the tolane unit and S3 (purple) can be assinged to a transition between the tolane and AB units.\\
While the ground state energy (blue) reaches its energetically highest point at \SI{100}{\degree} the singlet excited state S1 (orange) reaches its lowest point of energy at the same angle.
...
The slope of the ground state indicates that its maximum energy would be near a \SI{90}{\degree} CNNC angle, which coincides with the CNNC dihedral angle at which the conical intersection of AB is located.\cite{maximum-con-inter-AB} 
...
\\
The singlet excited state S2 can be assinged to a $\pi\pi^*$ transition located entirely on the AB unit, while S3 is a $\pi\pi^*$ transition from the tolane unit to the AB substructure, which is similar to a charge-transfer transition.
S2 seems to slightly increase in energy, while S3 sligthly decreases in energy respective to an increasing CNNC dihedral angle until an angle of \SI{100}{\degree} is reached, at which both seem to decrease in energy quite similarly.
The singlet excited state S6 can be assinged to the $\pi\pi^*$ transition at the tolane unit and its energy curve seems to mirror the groundstate albeit at a higher energy.
S6 seems to be energetically more stable compared to S2 and S3 at a low CNNC dihedral angle, while possessing a higher energy value at an CNNC dihedral angle of \SI[]{169}{\degree}.
%
%
% In figure \ref{fig:tors_tAB-cAB-1c}, the relative energies of the electronic states of tAB-1c-tolane-rot9 and cAB-1c-tolane-tor are plotted against the CNNC angle,
% while the GS energy of tAB-1c-tolane-rot9 with a CNNC dihedral angle of \SI[]{169}{\degree} is used as a reference point. 
% The n(AB)~$\pi^{*}$(AB) (orange) and the $\pi$(AB)~$\pi^{*}$(AB) (red) transitions take place at the AB substructure of the linked molecules. 
% Transition $\pi$(tol)~$\pi^{*}$(tol) (purple) is located at the tolane unit and the $\pi$(tol)~$\pi^{*}$(AB) transition describes a transition between the tolane and AB units.\\
% While the ground state energy (blue) reaches its energetically highest point at \SI{100}{\degree} the n(AB)~$\pi^{*}$(AB) state reaches its lowest point of energy at the same angle. 
% The resulting energy gap of \SI{0.4}{\eV} is still quite large which is mainly due to the inability to obtain constrained geometries between \SI{80}{\degree} and \SI{100}{\degree}. 
% The slope of the ground state indicates that its maximum energy would be near a \SI{90}{\degree} CNNC angle, which coincides with the CNNC dihedral angle at which the conical intersection of AB is located.\cite{maximum-con-inter-AB} 
% Thus one can assume that the conical intersection of tAB-1c-tolane-rot9 and cAB-1c-tolane-tor is still present in AB-1c-tolane, facilitating a possible switching event of the AB subsystem.
%
...
\begin{figure}[H]
    \centering
    \includegraphics[scale=0.75]{Figures/Torsion_s0_s1_s2/Pictures_AB-3c-tors_corr_2.eps}
    \caption{Relative energies of the GS and the singlet excited states in in relation to the CNNC dihedral angle of tAB-3c-tolane-rot3 and cAB-3c-tolane-tor.
    S1-S6 describe the singlet excited states of tAB-3c-tolane-rot3.}
    \label{fig:tors-tAB-cAB-3c}
\end{figure}
%
The vertical excitation energies of tAB-3c-tolane-rot3 and cAB-3c-tolane-tor were plotted against the CNNC dihedral angle (figure \ref{fig:tors-tAB-cAB-3c}), 
while the GS energy of tAB-3c-tolane-rot3 with a CNNC dihedral angle of \SI[]{179}{\degree} is used as a reference point.\\
The singlet excited states S1 (orange) and S2 (red) could be assigned to transitions at the AB substructure, whereas S2 starts at a tolane centered MO and terminates at an AB centered MO, quite similar to a charge-transfer transition. \\
...
%
The difference in energy between S2 and S3 is pronounced (\SI{0.95}{\eV} @\SI{8}{\degree}) at a small CNNC dihedral angle, but narrows as the angle increases (\SI{0.09}{\eV} @\SI{179}{\degree}). 
% The second and third excited states seem to decrease in energy respective to an increasing CNNC dihedral angle and don't vary much from each other in terms of energy.
S6, the $\pi\pi^*$ transition limited to the tolane unit, increases in energy up to a CNNC dihedral angle of \SI[]{80}{\degree} at which it decreases again, quite similar to the ground state. 
%
%
%
%
...
From the optimized structure calculations in section \ref{abs:iso_process} the MO energies were extracted.
%
...
% To reach this goal, the CNNC-angle of both the tAB-molecule and the cAB-molecule was varied in steps of \SI{20}{\degree} and the resulting geometry was optimized until the calculation no longer succeeded. 
% The frontier orbitals were calculated at each constrained geometry and their energy was plotted in relation to their CNNC-angle.\\
%
\begin{figure}[H]
    \centering
...
\end{figure}
%
Figure \ref{fig:MO_tors_1c_AB} shows the energies of LUMO+1, the LUMO, the HOMO and the HOMO-1 of tAB-1c-tolane-rot9 and cAB-1c-tolane-tor in relation to the CNNC torsional angle. \\ 
The energy values of both frontier orbitals which are located on tolane, $\pi^*$(tol) and $\pi$(tol), appear to be unaffected by the change of the CNNC dihedral angle.
% The energy values of both frontier orbitals which are located on tolane, $\pi^*$(tol) and $\pi$(tol), one observes, that the torsional deformation of the molecule has no effect on their energies. 
This is quite remarkable, because the dihedral angle of tolane while in tAB-1c-tolane-rot9 configuration is \SI{51}{\degree} whereas in cAB-1c-tolane-tor configuration this angle amounts only to \SI{15}{\degree}, 
implying that the dihedral angle has only a negligible influence on the energies of both tolane-localized frontier orbitals.\\
...
For the AB centered frontier orbitals the opposite is true. 
The energy value for $\pi^*$(AB) seems to decrease and reaches its minimum energy of \SI{2.24}{\eV} at a torsional angle of \SI{100}{\degree}, while the energy of n(AB) increases to a maximum of \SI{6.19}{\eV} at the same angle. 
The gap between both MOs decreases from \SI{6.69}{\eV} for the cAB-1c-tolane-tor to \SI{3.95}{\eV} at a torsional angle of \SI{100}{\degree}. 
%
%
%
%
...
The trend in energy of both LUMOs seems quite similar to the one observed in figure \ref{fig:MO_tors_1c_AB}. 
$\pi^*$(tol) possesses a higher energy in \textit{trans}-configuration than in \textit{cis}-configuration.
One possible explanation could be the reduced overlap of the orbitals located at the tolane unit, due to an increase of the \ce{Ph-C\bond{#}C-Ph} dihedral angle of tolane from \SI{3}{\degree} in \textit{cis}-configuration to \SI{21}{\degree} in \textit{trans}-configuration. 
% 
% by the increase of the dihedral angle of tolane from \SI{3}{\degree} in cis configuration to \SI{21}{\degree} in trans configuration. of tolane from \SI{3}{\degree} in cis configuration to \SI{21}{\degree} in trans configuration. 
$\pi^*$(AB) decreases in energy as the CNNC dihedral angle increases until its energetical minimum at \SI{80}{\degree} is reached, at which point it increases in energy again.  \\
% Interestingly enough the HOMO-1 of tAB-3c-tolane seems to include the pi orbitals at the azo bridge, but as soon as the torsional angle is decreased to \SI{160}{\degree} the HOMO-1 deforms to include n orbitals at the azo bridge. 
In contrast, the energy of the n(AB\_1) MO increases in relation to the CNNC dihedral angle until it reaches its maximum at \SI{80}{\degree} and then decreases to its minimum energy value at at \SI{179}{\degree}.
% 
% 
% The energy of the n(AB_1) MO increases in relation to the CNNC dihedral angle until it reaches its maximum at \SI{80}{\degree} and then decreases to its energetical minimum at at \SI{179}{\degree}.
% 
% The curve of n\_AB increases until it reaches its maximum at \SI{80}{\degree}. \\
The energy of both $\pi$(tol) and $\pi$(AB\_ph+tol\_ph)\_1 seems to increase respective to an increasing CNNC dihedral angle.
%
% $\pi$(tol) and $\pi$(AB\_ph+tol\_ph)\_1 and 2 both show a similar trend and decrease in energy from higher torsional angles to lower ones. 
% An interesting case is n(AB\_2) which shape includes n orbitals at higher degrees of torsion but loses those at lower torsional angles until only the pi orbitals located on the phenyl rings adjacent to the azo bridge remain. \\
\\
To summarize, a change in the CNNC dihedral angle affects the energy of the n(AB\_1) and $\pi^*$(AB) the most, leading to a respective energetic maximum and minimum at \SI[]{80}{\degree}.
The energy value of $\pi^*$(AB) is greater than $\pi^*$(tol) in \textit{cis}-configuration, but $\pi^*$(tol) possesses a greater energy value than $\pi^*$(AB) in \textit{trans}-configuration.
This observation could imply that $\pi^{*}$(tol) is more electropilic than $\pi^{*}$(AB) when AB-3c-tolane possesses \textit{cis}-configuration, but in \textit{trans}-configuration the reverse case occurs.
% This observation could potentially be used to target the AB unit of the molecule in \textit{cis}-configuration and the tolane unit in \textit{trans}-configuration with a nucleophilic reaction. 
% In the case of cAB-3c-tolane $\pi^*$(AB) is higher in energy than the $\pi^*$(tol), contrary to the \textit{trans}-isomer, in which case $\pi^*$(tol) possess a higher energy value than $\pi^*$(AB).
% This observation could potentially be used to target the AB unit of the molecule in \textit{cis}-configuration and the tolane unit in \textit{trans}-configuration with a nucleophilic reaction. 
% 
% Due to the fact that $\pi^*$(AB) is higher in energy than the $\pi^*$(tol) at cis-
% - $\pi^*$(AB) higher in energy than $\pi^*$(tol) in cis configuration
% - trans isomer the reverse case
% Since LUMO and LUMO+1 differ in their shape and location (AB or tolane) between \textit{cis} and \textit{trans} configuration, one could selectively target the azobenzene or the tolane fragment with certain chemical reactions depending on the specific isomer. 
...
% Figure \ref{fig:tAB-cAB-3c-tors_1} shows LUMO+1, LUMO and the five energetically highest occupied MOs of tAB-3c-tolane-rot3 and their change in energy at varying degrees of torsion. \\
% The trend in energy of both LUMOs seems quite similar to the one observed in figure \ref{fig:MO_tors_1c_AB}. 
% $\pi^*$(tol) possesses a higher energy in the trans configuration than cis configuration which can be explained by the increase of the dihedral angle of tolane from \SI{3}{\degree} in cis configuration to \SI{21}{\degree} in trans configuration. 
% $\pi^*$(AB) decreases in energy with a decreasing angle of torsion until it reaches its minimum at \SI{80}{\degree}. \\
% Interestingly enough the HOMO-1 of tAB-3c-tolane seems to include the pi orbitals at the azo bridge, but as soon as the torsional angle is decreased to \SI{160}{\degree} the HOMO-1 deforms to include n orbitals at the azo bridge. 
% The curve of n\_AB increases until it reaches its maximum at \SI{80}{\degree}. \\
% $\pi$(tol) and $\pi$(AB\_ph+tol\_ph)\_1 and 2 both show a similar trend and decrease in energy from higher torsional angles to lower ones. 
% An interesting case is n(AB\_2) which shape includes n orbitals at higher degrees of torsion but loses those at lower torsional angles until only the pi orbitals located on the phenyl rings adjacent to the azo bridge remain. \\
% \\
% To summarize one can see that the torsion mostly influences AB centered MOs, especially the ones higher in energy like n(AB\_1) and $pi^*$(AB). Since LUMO and LUMO+1 differ in their shape and location (AB or tolane) between \textit{cis} and \textit{trans} configuration, one could selectively target the azobenzene or the tolane fragment with certain chemical reactions depending on the specific isomer. 
% Another important aspect is the change in energy of the LUMO and LUMO+1 at the cis configuration which could be used to selectively target the tolane or azobenzene with certain electrophilic reactions. Similarly could the change in shape of the HOMO and HOMO-1 between the trans and cis configuration be used to selectively target specific fragments with nucleophilic reactions.
%
%
%
...
\end{subfigure}
\caption{The geometries of tAB-3c-tolane-flat \textbf{(a)} and tAB-3c-tolane \textbf{(b)}.}
...
\toprule
\multicolumn{1}{l}{Molecule (with D3)} & \multicolumn{1}{c}{Solvent} & \multicolumn{1}{c}{$H$ $\left(\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}\right)$} & \multicolumn{1}{c}{$S$ $\left(\frac{\SI{}{\joule}}{\SI{}{\mole\kelvin}}\right)$} &  \multicolumn{1}{c}{$G$ $\left(\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}\right)$}\\ \midrule
...
\end{table}
%
In the case of tAB-3c-tolane the solvents possessing a higher polarity lead to a lower enthalpy as well as a lower entropy, albeit the differences are relatively minor. 
Quite the opposite trend can be observed for tAB-3c-tolane-flat in which the entropy reaches its maximum in toluene and differs about $\SI{29}{\frac{\SI{}{\joule}}{\SI{}{\mole\kelvin}}}$ from acetronitrile as well as dichloromethane. 
The Gibbs free energy of tAB-3c-tolane is the lowest in DCM, but the difference between the other solvents is small (0.6 $\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}$).
tAB-3c-tolane-flat possesses an even lower value for the Gibbs free energy in toluene compared to tAB-3c-tolane in DCM and the difference between the other solvents is greater (6.4 $\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}$).
% 
% A lower Gibbs free energy can be observed for tAB-3c-tolane-flat in toluene compared to tAB-3c-tolane, while the difference between the other solvents is greater (6.4 $\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}$).
% For tAB-3c-tolane-flat, the smallest value for the Gibbs free energy results from the pcm calculation with toluene,  % with a respective deviation of ~6.5 $\frac{\SI{}{\kilo\joule}}{\SI{}{\mole}}$ between the other solvents.
% which is lower 
\\
One explanation for the stark contrast in entropy of tAB-3c-tolane-flat dissolved in toluene and DCM or acetronitrile could be the low polarity of toluene compared with the remaining solvents. 
Toluene possesses a dielectric constant of 2.38 in contrast to acetonitrile (36.64) and DCM (9.08).\cite{Dieelectric-constants} 
Due to the nature of the C-PCM solvation model non-electrostatic effects (cavitation, dispersion and repulsion) are being disregarded which leads to the assumption that solutions with low dielectric constants interact with the molecule less than those with high dielectric constants. 
Thus solvents like DCM and acetonitrile seem to lower the total entropy of a dissolved molecule due to the nature of their stronger electrostatic interaction with it. \\
The question why this effect appears only in tAB-3c-tolane-flat and not in tAB-3c-tolane-rot3 could be answered by looking at the higher surface area of the flat molecule in comparison to the stacked variant. 
This could lead to more interactions between the solution and the dissolved molecule and thus lead to a different entropy.
...
\end{table}
%
To grasp the influence of dispersion on the trends observed in the prior calculations of those two molecules, we performed the same calculations which lead to table \ref{tab:thermo_D3} without an additional dispersion correction. 
The results of those calculations are presented in table \ref{tab:thermo_noD3}.\\
...
%
\\
The CREST software was successfully used to sample through a wide range of conformers. 
Their GS geometries were then optimized at the CAM-B3LYP/6-311G* + D3(BJ) level of theory for each \textit{trans}-azobenzene molecule a \textit{cis}-azobenzene molecule was generated, which served as the likely endpoint of a \textit{trans}-\textit{cis}-isomerisation. 
%
%
% GS calc
\begin{comment}
...
%
% Excited state
Analyzing their respective excited state revealed that the first excited state describes the same n$\pi$* transition centered around the AB substructure of the molecules. 
Differences appear in the following excited states: 
For the molecules possessing \textit{trans}-geometry the $\pi\pi$* centered around AB are lower in energy than the following tolane centered $\pi\pi$* transition. 
An exception is the S2 of tAB-3c-tolane-rot9 describing a charge-transfer transition between the HOMO centered at the tolane backbone and the tAB centered LUMO which likely arises due to the staggered stacked conformation of tAB-3c-tolane-rot9. \\
The opposite case can be observed from the \textit{cis}-molecules, cAB-1c-tolane-tor and cAB-3c-tolane-tor. 
Their excited states describing the $\pi\pi$* transitition centered around tolane seem to be lower in energy than the AB $\pi\pi$* transitions. 
The reason for this is probably the deformed $\pi$* orbitals which increase the energy of the transition. 
%
%
\begin{comment}
- tAB-1c-tolane:
...
\end{comment}
%
Plotting the isomerisation pathway reveals a likely retained switching ability of the AB system.
%no direct crossing between GS and S1 which is likely due to the SCF not converging around a CNNC-angle between \SI{80}{\degree} and \SI{100}{\degree}. 
%The slope of the GS and S1 indicate that a possible CI is likely at \SI{90}{\degree}, similar to azobenzene.
%
%isomerisation
\begin{comment}
...
\end{comment}
%
The energies of the tolane centered orbitals of the methylene- and propylene-linked molecules seem rather unaffected by the changing CNNC-angle compared to those centered on the AB. 
This seems quite plausible since the biggest structural change is found at the AB substructure during isomerisation while the tolane backbone only deforms slightly. 
% On the contrary both $\pi$(AB) and $\pi$*(AB) orbitals are strongly affected resulting in a HOMO-LUMO gaps of \SI{3.95}{\eV} (methylene-linked molecule) and \SI{4.19}{\eV} (propylene-linked molecule). 
It appears that the tolane centered $\pi^*$ of tAB-3c-tolane-rot9 is higher in energy than the AB-centered one, but this energetic order reverses for the \textit{cis}-isomer cAB-3c-tolane-tor, which could have implications for reactions. \\
%targeting those molecular orbitals.\\
...
\end{comment}
%
The thermodynamic calculations, including the C-PCM solvent model, showed that 
%the different solutions had only a negligible influence on the entropy and enthalpy values of tAB-3c-tolane-rot3. 
%On the other hand one could observe a lower entropy value regarding the acetonitrile solvent for the tAB-3c-tolane-flat molecule. 
%It appears that
polar solvents seem to lower the molecular entropy for the flat molecule, but not for tAB-3c-tolane-rot3, which could be due to tAB-3c-tolane-rot3 possessing less surface area than tAB-3c-tolane-flat and thus having a smaller area which can interact with the solvent. \\
%
%thermodynamics
%
\begin{comment}
